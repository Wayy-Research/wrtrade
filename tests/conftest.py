...
def ma_crossover_signal():
    ...
    def signal_func(
        prices: pl.Series, fast_period: int = 10, slow_period: int = 30
    ) -> pl.Series:
        ...
        for i in range(len(prices)):
            if i < slow_period:
                ...
                prev_fast = (
                    fast_ma[i - 1] if i > 0 and fast_ma[i - 1] is not None else 0
                )
                prev_slow = (
                    slow_ma[i - 1] if i > 0 and slow_ma[i - 1] is not None else 0
                )
...
def momentum_signal():
    ...
    def signal_func(
        prices: pl.Series, lookback: int = 20, threshold: float = 0.05
    ) -> pl.Series:
...
def multi_market_prices():
    ...
    correlation_matrix = np.array([[1.0, 0.6, 0.4], [0.6, 1.0, 0.3], [0.4, 0.3, 1.0]])
    ...
    random_returns = np.random.multivariate_normal(
        mean=np.zeros(n_markets), cov=correlation_matrix, size=n_days
    )
...
def mock_api_credentials():
    ...
    return {"api_key": "test_api_key_12345", "secret_key": "test_secret_key_67890"}
