...
class TestKellyConfig:
    ...
    def test_custom_config(self):
        ...
        config = KellyConfig(
            min_weight=-0.5, max_weight=2.0, max_leverage=3.0, risk_free_rate=0.05
        )
...
class TestKellyOptimizer:
    ...
    def constrained_optimizer(self):
        ...
        config = KellyConfig(
            min_weight=0.1, max_weight=0.9, max_leverage=1.0, allow_short=False
        )
    ...
    def test_constrained_kelly_optimization(
        self, constrained_optimizer, multi_market_prices
    ):
        ...
        constrained_weights = constrained_optimizer.calculate_portfolio_kelly(
            returns_matrix
        )
...
class TestHierarchicalKellyOptimizer:
    ...
    def hierarchical_optimizer(self):
        ...
        config = KellyConfig(
            min_weight=0.0, max_weight=1.0, lookback_window=100  # Shorter for testing
        )
    ...
    def test_single_component_optimization_interface(
        self, hierarchical_optimizer, sample_signal_components
    ):
    ...
    def test_portfolio_interface_check(
        self, hierarchical_optimizer, sample_signal_components, long_sample_prices
    ):
        ...
        portfolio = CompositePortfolio(
            name="test_portfolio", components=sample_signal_components[:2], weight=1.0
        )
...
class TestKellyIntegration:
    ...
    def test_kelly_with_simple_portfolio(
        self, sample_signal_components, long_sample_prices
    ):
        ...
        # Create portfolio
        portfolio = CompositePortfolio(
            name="kelly_test", components=sample_signal_components[:2], weight=1.0
        )
    ...
    def test_kelly_config_variations(
        self, sample_signal_components, long_sample_prices
    ):
        ...
        portfolio = CompositePortfolio(
            name="config_test", components=sample_signal_components[:2], weight=1.0
        )
    ...
    def test_kelly_numerical_stability(self):
        ...
        for i, returns_data in enumerate(extreme_cases):
            ...
            assert (
                abs(kelly)
                <= optimizer.config.max_weight * optimizer.config.max_leverage + 1e-6
            )
