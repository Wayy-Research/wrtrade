...
class TestPricePermutationGenerator:
    ...
    def test_permutation_statistical_properties(self, generator, long_sample_prices):
        ...
        permuted_prices = generator.generate_single_market_permutation(
            long_sample_prices
        )
        ...
        assert (
            abs(original_mean - permuted_mean) < 0.001
        ), f"Mean difference too large: {abs(original_mean - permuted_mean)}"
        assert (
            abs(original_std - permuted_std) < 0.005
        ), f"Std difference too large: {abs(original_std - permuted_std)}"
    ...
    def test_permutation_reproducibility(self, sample_prices):
        ...
        np.testing.assert_array_almost_equal(
            perm1.to_numpy(), perm2.to_numpy(), decimal=8
        )
    ...
    def test_multi_market_permutation(self, generator, multi_market_prices):
        ...
        permuted_markets = generator.generate_multi_market_permutation(
            multi_market_prices
        )
        ...
        for i, (original, permuted) in enumerate(
            zip(multi_market_prices, permuted_markets)
        ):
        ...
        for i in range(len(multi_market_prices)):
            for j in range(i + 1, len(multi_market_prices)):
                ...
                assert (
                    abs(orig_corr - perm_corr) < 0.15
                ), f"Correlation change too large: {orig_corr:.3f} -> {perm_corr:.3f}"
    ...
    def test_empty_price_series(self, generator):
        ...
        assert (
            abs(short_result[0] - short_prices[0]) < 1e-10
    ...
    def _calculate_correlation_matrix(
        self, price_series_list: List[pl.Series]
    ) -> List[List[float]]:
...
class TestPermutationTester:
    ...
    def test_insample_permutation_test(
        self, tester, sample_prices, simple_strategy_func
    ):
        ...
        assert np.isfinite(
            results["real_performance"]
        ), "Real performance should be finite"
    ...
    def test_walkforward_permutation_test(
        self, tester, long_sample_prices, simple_strategy_func
    ):
    ...
    def test_multi_market_permutation_testing(self, tester, multi_market_prices):
        ...
        assert np.isfinite(results["real_performance"]) or results[
            "real_performance"
        ] == float("inf")
...
class TestPermutationTestingIntegration:
    ...
    def test_portfolio_permutation_test(
        ...
        ma_component = builder.create_signal_component(
            "ma_signal", ma_crossover_signal, 0.6
        )
        momentum_component = builder.create_signal_component(
            "momentum_signal", momentum_signal, 0.4
        )

        portfolio = builder.create_portfolio(
            "test_portfolio", [ma_component, momentum_component]
        )
        ...
        assert (
            abs(results["real_performance"]) > 1e-6
            or results["real_performance"] == 0.0
        )

    def test_nested_portfolio_permutation_test(
        self, long_sample_prices, ma_crossover_signal, buy_hold_signal
    ):
        ...
        main_portfolio = builder.create_portfolio(
            "main_portfolio", [sub_portfolio, signal2]
        )
    ...
    def test_permutation_test_significance_detection(self, sample_prices):
        ...
        random_results = tester.run_insample_test(
            prices=sample_prices, strategy_func=random_strategy, metric="sortino_ratio"
        )
        ...
        def perfect_strategy(prices, **kwargs):
            ...
            perfect_signals = (price_changes > 0).cast(pl.Float64) - (
                price_changes < 0
            ).cast(pl.Float64)
            perfect_returns = (
                perfect_signals.shift(1).fill_null(0) * price_changes / prices
            )
            return perfect_returns

        perfect_results = tester.run_insample_test(
            prices=sample_prices, strategy_func=perfect_strategy, metric="sortino_ratio"
        )
    ...
    def test_permutation_config_impact(self, sample_prices, ma_crossover_signal):
        ...
        assert (
            abs(results[0]["real_performance"] - results[1]["real_performance"]) < 1e-10
        )
