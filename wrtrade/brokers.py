...
class AlpacaBrokerAdapter(BrokerAdapter):
    ...
    async def get_account_info(self) -> AccountInfo:
        ...
        try:
            ...
            account_response = requests.get(
                f"{self.base_url}/account", headers=self.headers
            )
            ...
            return AccountInfo(
                ...
                day_trade_buying_power=float(
                    account_data.get("daytrading_buying_power", 0)
                ),
    ...
    async def place_order(self, order: Order) -> OrderStatus:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/orders", headers=self.headers, json=order_data
            )
            response.raise_for_status()
            ...
            order_status = OrderStatus(
                ...
                remaining_quantity=float(order_response.get("qty", 0))
                - float(order_response.get("filled_qty", 0)),
                average_fill_price=(
                    float(order_response.get("filled_avg_price", 0))
                    if order_response.get("filled_avg_price")
                    else None
                ),
                created_at=datetime.fromisoformat(
                    order_response["created_at"].replace("Z", "+00:00")
                ),
                updated_at=datetime.fromisoformat(
                    order_response["updated_at"].replace("Z", "+00:00")
                ),
            )
    ...
    async def get_order_status(self, order_id: str) -> OrderStatus:
        ...
        try:
            ...
            return OrderStatus(
                ...
                remaining_quantity=float(order_data.get("qty", 0))
                - float(order_data.get("filled_qty", 0)),
                average_fill_price=(
    ...
    async def get_historical_data(
        ...
        try:
            ...
            response = requests.get(
                f"{self.data_url}/stocks/bars", headers=self.headers, params=params
            )
            response.raise_for_status()
...
class RobinhoodBrokerAdapter(BrokerAdapter):
    ...
    def __init__(self, api_credentials: Dict[str, str], paper_trading: bool = True):
        ...
        logger.warning(
            "Robinhood API is not officially available. This is a placeholder implementation."
        )
...
class BrokerFactory:
    ...
    @staticmethod
    def create_broker(
        broker_name: str, api_credentials: Dict[str, str], paper_trading: bool = True
    ) -> BrokerAdapter:
        """
    ...
    async def execute_portfolio_rebalance(
        ...
        for symbol, target_value in target_positions.items():
            ...
            if abs(value_diff) > 100:  # Only rebalance if difference > $100
                ...
                current_price = (
                    market_data[symbol]["bid"] + market_data[symbol]["ask"]
                ) / 2
        ...
        for order in orders_to_place:
            try:
                ...
                if self.require_confirmation:
                    confirmation = input(
                        f"Execute {order.side} {order.quantity} {order.symbol}? (y/n): "
                    )
    ...
    def _check_risk_limits(self, order: Order, account_info: AccountInfo) -> bool:
        ...
        if order_value > max_order_value:
            self.risk_breaches.append(
                f"Position size limit exceeded for {order.symbol}"
            )
            return False
        ...
        while True:
            try:
                ...
                total_unrealized_pnl = sum(
                    pos.unrealized_pnl for pos in account_info.positions
                )
                self.daily_pnl = (
                    total_unrealized_pnl  # Simplified - should track realized too
                )
                ...
                if self.daily_pnl < -self.max_daily_loss * account_info.portfolio_value:
                    logger.warning(
                        "DAILY LOSS LIMIT EXCEEDED - Consider closing positions"
                    )
