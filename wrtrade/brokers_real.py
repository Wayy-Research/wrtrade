...
class AlpacaBrokerAdapter(BrokerAdapter):
    ...
    def __init__(
        ...
        self.base_url = api_credentials.get(
            "base_url",
            (
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Alpaca authentication successful. Account: {account.account_number}"
            )
            return True
    ...
    async def place_order(self, order: Order) -> bool:
        ...
        try:
            ...
            if (
                hasattr(submitted_order, "filled_avg_price")
                and submitted_order.filled_avg_price
            ):
    ...
    async def get_historical_data(
        ...
        try:
            ...
            logger.info(
                f"Requesting {symbol} data from {start_str} to {end_str} (timeframe: {timeframe})"
            )

            # Get bars
            bars_request = self.client.get_bars(
                symbol, tf, start=start_str, end=end_str, adjustment="raw"
            )
            ...
            logger.error(
                f"Request details - Symbol: {symbol}, Start: {start_str}, End: {end_str}, Timeframe: {timeframe}"
            )
            return pl.DataFrame()
...
class RobinhoodBrokerAdapter(BrokerAdapter):
    ...
    def __init__(
        ...
        if not ROBINHOOD_AVAILABLE:
            raise ImportError(
                "Robin Stocks not available. Install with: pip install robin-stocks"
            )
    ...
    async def place_order(self, order: Order) -> bool:
        ...
        try:
            ...
            rh_params = {
                ...
                "side": order.side,
                "timeInForce": (
                    "gfd" if order.time_in_force == TimeInForce.DAY else "gtc"
                ),
            ...
            if order.order_type == OrderType.MARKET:
                ...
                logger.error(
                    f"Order type {order.order_type} not supported by Robinhood adapter"
                )
                return False
                    ...
                    data.append(
                        {
                            "timestamp": datetime.fromisoformat(
                                hist["begins_at"].replace("Z", "+00:00")
                            ),
