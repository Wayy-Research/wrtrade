...
from wrtrade.local_deploy import (
    LocalDeployment,
    StrategyManager,
    StrategyConfig,
    StrategyStatus,
...
@click.option(
    "--symbols", "-s", help="Trading symbols (comma-separated)", required=False
)
@click.option(
    "--max-position", type=float, default=1000.0, help="Maximum position size ($)"
)
@click.option(
    "--risk-per-trade", type=float, default=0.02, help="Risk per trade (fraction)"
)
@click.option("--max-trades", type=int, default=10, help="Maximum daily trades")
@click.option(
    "--cycle-minutes", type=int, default=5, help="Strategy cycle interval (minutes)"
)
@click.option(
    "--market-hours-only",
    is_flag=True,
    default=True,
    help="Trade during market hours only",
)
@click.option(
    "--kelly-optimization", is_flag=True, default=True, help="Enable Kelly optimization"
)
@click.option(
    "--permutation-testing",
    is_flag=True,
    default=False,
    help="Enable permutation testing",
)
@click.option(
    "--auto-restart", is_flag=True, default=True, help="Auto-restart on failure"
)
...
def deploy(
    ...
    kelly_optimization: bool,
    permutation_testing: bool,
    auto_restart: bool,
    config: Optional[str],
):
    ...
    try:
        ...
        if config:
            ...
            config_dict = {
                ...
                "symbols": (
                    symbols.split(",")
                    if symbols
                    else get_env("TRADING_SYMBOLS", "AAPL,TSLA", list)
                ),
                ...
                "cycle_interval_minutes": get_env(
                    "CYCLE_INTERVAL_MINUTES", cycle_minutes, int
                ),
                "market_hours_only": get_env(
                    "MARKET_HOURS_ONLY", market_hours_only, bool
                ),
                "enable_kelly_optimization": get_env(
                    "ENABLE_KELLY_OPTIMIZATION", kelly_optimization, bool
                ),
                "enable_permutation_testing": get_env(
                    "ENABLE_PERMUTATION_TESTING", permutation_testing, bool
                ),
        ...
        if config_dict["broker_name"].lower() not in available_brokers:
            click.echo(
                f"❌ Error: Broker '{config_dict['broker_name']}' not available."
            )
...
def _print_strategy_summary(data: Dict[str, Any]):
    ...
    click.echo(
        f"{status_emoji} {data['name']}: {data['status'].upper()}{uptime}{trades}"
    )
...
@click.argument("name")
@click.option(
    "--remove-data", is_flag=True, help="Remove all data (logs, configs, etc.)"
)
def remove(name: str, remove_data: bool):
    ...
    try:
        ...
        if remove_data:
            click.echo(
                f"⚠️  This will remove strategy '{name}' and ALL its data (logs, configs, etc.)"
            )
        else:
...
def init():
    ...
    try:
        ...
        click.echo(
            f"🚀 Deploy a strategy with: wrtrade strategy deploy <strategy.py> --name my-strategy"
        )
...
@click.option(
    "--symbol", "-s", default="BTC-USD", help="Trading symbol (default: BTC-USD)"
)
@click.option(
    "--cash", "-c", type=float, default=10000.0, help="Initial cash (default: $10,000)"
)
...
def deploy(
    ...
    try:
        ...
        if result["status"] == "running":
            ...
            click.echo(
                f"   Check status: wrtrade wayyfin status {result['strategy_id'][:12]}"
            )
        else:
            click.echo()
            click.echo(
                f"   Start trading: wrtrade wayyfin start {result['strategy_id'][:12]}"
            )
    ...
    try:
        ...
        if strategy_id:
            ...
            for entry in result.get("leaderboard", []):
                ...
                click.echo(
                    f"#{rank:<3} {name:<20} {symbol:<8} Sortino: {sortino:>6.2f}  Return: {ret*100:>6.1f}%  {live}"
                )
...
def leaderboard(url: str):
    ...
    try:
        ...
        click.echo("=" * 70)
        click.echo(
            f"{'#':<4} {'Name':<22} {'Symbol':<9} {'Sortino':>8} {'Return':>9} {'Status':<6}"
        )
        click.echo("-" * 70)
        ...
        for entry in result.get("leaderboard", []):
            ...
            name = (
                ...
                if len(entry.get("name", "")) > 20
                else entry.get("name", "Unknown")
            )
            symbol = entry.get("symbol", "-")
            sortino = entry.get("display_sortino", 0) or 0
            ...
            status = (
                "LIVE"
                if entry.get("is_live_trading")
                else entry.get("metrics_source", "-")[:6]
            )

            click.echo(
                f"#{rank:<3} {name:<22} {symbol:<9} {sortino:>8.2f} {ret:>8.1f}% {status:<6}"
            )
