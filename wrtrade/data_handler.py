...
class UnifiedDataHandler:
    ...
    def define_mixed_portfolio(
        self, stocks: List[str] = None, etfs: List[str] = None, crypto: List[str] = None
    ) -> List[AssetInfo]:
        """
        ...
        if etfs:
            portfolio.extend(
                [AssetInfo(symbol, AssetType.ETF, f"{symbol} ETF") for symbol in etfs]
            )
    ...
    async def get_historical_data(
        ...
        try:
            if asset.asset_type == AssetType.CRYPTO:
                data = await self._get_crypto_historical_data(
                    asset, start_date, end_date, timeframe
                )
            else:  # STOCK or ETF
                data = await self._get_stock_historical_data(
                    asset, start_date, end_date, timeframe
                )
            ...
            self.logger.error(
                f"Failed to get historical data for {asset.display_name}: {e}"
            )
            return pl.DataFrame()

    async def _get_crypto_historical_data(
        self, asset: AssetInfo, start_date: datetime, end_date: datetime, timeframe: str
    ) -> pl.DataFrame:
        ...
        self.logger.info(
            f"Requesting crypto data: {asset.symbol} from {start_str} to {end_str}"
        )
        ...
        self.logger.info(
            f"Alpaca returned {len(bars_df)} crypto bars for {asset.symbol}"
        )

        if len(bars_df) > 0:
            self.logger.debug(
                f"Crypto data sample for {asset.symbol}: {bars_df.head()}"
            )
    ...
    async def _get_stock_historical_data(
        ...
        return await self.broker.get_historical_data(
            asset.symbol, start_date, end_date, timeframe
        )
    ...
    async def get_market_data(self, assets: List[AssetInfo]) -> Dict[str, MarketData]:
        ...
        stock_assets = [
            a for a in assets if a.asset_type in (AssetType.STOCK, AssetType.ETF)
        ]
    ...
    async def _get_crypto_market_data(
        self, crypto_assets: List[AssetInfo]
    ) -> Dict[str, MarketData]:
        ...
        try:
            ...
            for symbol in crypto_symbols:
                if hasattr(snapshots, symbol.replace("/", "").lower()):
                    ...
                    if (
                        snapshot
                        and hasattr(snapshot, "latest_quote")
                        and snapshot.latest_quote
                    ):
    ...
    async def get_portfolio_historical_data(
        ...
        """
        self.logger.info(
            f"Fetching historical data for {len(portfolio)} assets in mixed portfolio"
        )
