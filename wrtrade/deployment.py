...
class StrategyValidator:
    ...
    async def validate_strategy(
        self, portfolio: CompositePortfolio, prices: pl.Series, config: DeploymentConfig
    ) -> ValidationResult:
        """
        ...
        if len(prices) < config.min_backtest_period_days:
            errors.append(
                f"Insufficient historical data: {len(prices)} days < {config.min_backtest_period_days} required"
            )
        ...
        try:
            ...
            logger.info(
                f"Backtest completed. Sortino ratio: {backtest_metrics.get('sortino_ratio', 0):.3f}"
            )
        ...
        if sortino_ratio < config.min_sortino_ratio:
            errors.append(
                f"Sortino ratio too low: {sortino_ratio:.3f} < {config.min_sortino_ratio}"
            )
        ...
        try:
            ...
            permutation_config = PermutationConfig(
                n_permutations=200, parallel=False
            )  # Faster validation
            ...
            if permutation_pvalue > config.max_permutation_pvalue:
                errors.append(
                    f"Strategy failed permutation test: p-value {permutation_pvalue:.4f} > {config.max_permutation_pvalue}"
                )
...
class StrategyDeployer:
    ...
    async def deploy_strategy(
        ...
        logger.info(
            f"Starting deployment of strategy: {config.strategy_name} (ID: {deployment_id})"
        )
        ...
        try:
            ...
            broker = BrokerFactory.create_broker(
                config.broker_name, api_credentials, config.paper_trading
            )
            ...
            if config.kelly_optimization:
                ...
                optimization_results = kelly_optimizer.optimize_portfolio(
                    portfolio, prices
                )
                logger.info(
                    f"Kelly optimization completed for {len(optimization_results['level_optimizations'])} levels"
                )
        ...
        try:
            while status.status == "running":
                ...
                if days_since_rebalance >= config.rebalance_frequency_days:
                    ...
                    try:
                        ...
                        if config.kelly_optimization:
                            ...
                            logger.info(
                                "Kelly optimization scheduled (implementation pending data source)"
                            )
                        ...
                        logger.info(
                            f"Rebalancing completed: {len(orders)} orders executed"
                        )
                        last_rebalance = datetime.now()
                ...
                if self._should_auto_shutdown(status, config):
                    logger.warning(
                        f"Auto-shutdown triggered for deployment: {deployment_id}"
                    )
    ...
    async def _monitor_performance(
        self, deployment_id: str, trading_session: TradingSession
    ) -> None:
        ...
        try:
            ...
            status.max_drawdown = min(
                status.max_drawdown, status.daily_pnl / account_info.portfolio_value
            )

            logger.debug(
                f"Performance update for {deployment_id}: Daily P&L ${status.daily_pnl:.2f}"
            )
    ...
    def _should_auto_shutdown(
        self, status: DeploymentStatus, config: DeploymentConfig
    ) -> bool:
        ...
        if status.max_drawdown <= conditions.get("max_drawdown", -1.0):
            status.risk_alerts.append(
                f"Max drawdown exceeded: {status.max_drawdown:.3f}"
            )
            return True
        ...
        if status.daily_pnl <= daily_loss_limit * 100000:  # Assuming $100k portfolio
            status.risk_alerts.append(
                f"Daily loss limit exceeded: ${status.daily_pnl:.2f}"
            )
            return True
    ...
    def stop_deployment(self, deployment_id: str, reason: str = "Manual stop") -> bool:
        ...
        if status.status in ["running", "deployed"]:
            ...
            logger.warning(
                f"Cannot stop deployment {deployment_id} in status: {status.status}"
            )
            return False
    ...
    def get_deployment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_deployments": len(
                [s for s in statuses if s.status in ["running", "deployed"]]
            ),
...
class WRTradeDeploymentSystem:
    ...
    async def deploy_portfolio_strategy(
        ...
        if deployment_config is None:
            deployment_config = DeploymentConfig(
                strategy_name=portfolio.name, broker_name=broker_config["broker"]
            )
    ...
    def create_simple_strategy_example(self) -> CompositePortfolio:
        ...
        def ma_crossover_signal(prices: pl.Series) -> pl.Series:
            ...
            for i in range(len(prices)):
                if i < 30:  # Not enough data
                    signals.append(0)
                elif ma_short[i] > ma_long[i] and (
                    i == 0 or ma_short[i - 1] <= ma_long[i - 1]
                ):
        ...
        ma_component = SignalComponent(
            name="MA_Crossover", signal_func=ma_crossover_signal, weight=1.0
        )
