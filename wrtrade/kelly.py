...
class KellyOptimizer:
    ...
    def calculate_discrete_kelly(
        self, returns: pl.Series, risk_free_rate: Optional[float] = None
    ) -> float:
        """
        ...
        if len(returns_np) < self.config.min_observations:
            logger.warning(
                f"Insufficient data for Kelly calculation: {len(returns_np)} < {self.config.min_observations}"
            )
            return 0.0
    ...
    def calculate_portfolio_kelly(
        ...
        regularized_cov = cov_matrix + self.config.regularization_lambda * np.eye(
            n_assets
        )
        ...
        if self.config.use_constraints:
            ...
            kelly_weights = np.clip(
                kelly_weights, self.config.min_weight, self.config.max_weight
            )
    ...
    def _apply_weight_constraints(self, weights: np.ndarray) -> np.ndarray:
        ...
        if total_leverage > self.config.max_leverage:
            constrained_weights = constrained_weights * (
                self.config.max_leverage / total_leverage
            )

        return constrained_weights

    def calculate_fractional_kelly(
        self, weights: Dict[str, float], fraction: float = 0.25
    ) -> Dict[str, float]:
        """
    ...
    def calculate_dynamic_fraction(
        self, returns_matrix: np.ndarray, target_volatility: float = 0.15
    ) -> float:
        """
...
class HierarchicalKellyOptimizer:
    ...
    def optimize_portfolio(
        self, portfolio: CompositePortfolio, prices: pl.Series, rebalance: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Hierarchical optimization completed. Optimized {optimization_results['total_components_optimized']} components"
        )
    ...
    def _optimize_portfolio_level(
        ...
        try:
            ...
            kelly_weights = {
                name: 1.0 / len(component_names) for name in component_names
            }
        ...
        level_result = {
            ...
            "portfolio_name": portfolio.name,
            "original_weights": {
                comp.name: comp.weight for comp in portfolio.component_list
            },
    ...
    def _calculate_level_metrics(
        self, returns_matrix: np.ndarray, weights: Dict[str, float], names: List[str]
    ) -> Dict[str, float]:
    ...
    def _apply_optimization_results(
        self, portfolio: CompositePortfolio, results: Dict[str, Any]
    ) -> None:
    ...
    def calculate_kelly_growth_rate(
        self, returns_matrix: np.ndarray, weights: Dict[str, float]
    ) -> float:
        """
    ...
    def compare_allocations(
        ...
        comparison_results = {"methods": {}, "rankings": {}, "summary": {}}
        ...
        for method in allocation_methods:
            try:
                if method == "equal_weight":
                    ...
                    weights = self.optimizer.calculate_portfolio_kelly(
                        returns_matrix, component_names
                    )
                elif method == "kelly_quarter":
                    full_kelly = self.optimizer.calculate_portfolio_kelly(
                        returns_matrix, component_names
                    )
                    weights = self.optimizer.calculate_fractional_kelly(
                        full_kelly, 0.25
                    )
                ...
                metrics = {
                    ...
                    "sharpe_ratio": 0.0,
                    "max_drawdown": 0.0,
                    "kelly_growth_rate": self.calculate_kelly_growth_rate(
                ...
                if metrics["volatility"] > 0:
                    excess_return = (
                        np.mean(portfolio_returns) * 252 - self.config.risk_free_rate
                    )
                    metrics["sharpe_ratio"] = excess_return / metrics["volatility"]
