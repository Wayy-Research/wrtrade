...
from wrtrade.components import (
    PortfolioComponent,
    SignalComponent,
    CompositePortfolio,
    AllocationWeights,
...
class NDimensionalPortfolioBuilder:
    ...
    def register_signal(
        self, name: str, signal_func: Callable
    ...
    def create_balanced_portfolio(
        self, name: str, components: List[PortfolioComponent], weight: float = None
    ) -> CompositePortfolio:
        """
    ...
    def validate_portfolio_structure(
        self, portfolio: CompositePortfolio, depth: int = 0
    ) -> bool:
        """
        ...
        if depth > self.config.max_depth:
            raise ValueError(
                f"Portfolio depth exceeds maximum of {self.config.max_depth}"
            )
...
class AdvancedPortfolioManager:
    ...
    def backtest(self, prices: pl.Series) -> Dict[str, Any]:
        ...
        self._backtest_results = {
            ...
            "excess_return": portfolio_returns.cum_sum()[-1]
            - benchmark_returns.cum_sum()[-1],
        }

        logger.info(
            f"Backtest completed. Total return: {self._backtest_results['total_return']:.4f}"
        )
        ...
        def collect_contributions(portfolio: CompositePortfolio, prefix: str = ""):
            for component in portfolio.component_list:
                ...
                if hasattr(component, "_returns") and component._returns is not None:
                    ...
                    attribution["component_contributions"][full_name] = float(
                        contribution
                    )
    ...
    def run_permutation_test(
        ...
        logger.info(
            f"Permutation test will be implemented with {n_permutations} permutations"
        )
    ...
    def _get_performance_summary(self) -> Dict[str, float]:
        ...
        return {
            ...
            "sharpe_ratio": self._backtest_results["portfolio_metrics"].get(
                "sortino_ratio", 0.0
            ),
            "max_drawdown": self._backtest_results["portfolio_metrics"].get(
                "max_drawdown", 0.0
            ),
            "volatility": self._backtest_results["portfolio_metrics"].get(
                "volatility", 0.0
            ),
        }
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if performance["excess_return"] < 0:
            recommendations.append(
                "Portfolio underperforming benchmark - consider strategy review"
            )
        ...
        if performance["volatility"] > 0.3:
            recommendations.append(
                "High volatility - consider position sizing optimization"
            )
