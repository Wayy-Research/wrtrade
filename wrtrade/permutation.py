...
class PricePermutationGenerator:
    ...
    def generate_single_market_permutation(
        self, prices: pl.Series, start_index: Optional[int] = None
    ) -> pl.Series:
        """
        ...
        if start_index < n_bars - 1:
            ...
            for i in range(len(permuted_changes)):
                permuted_log_prices[start_index + 1 + i] = permuted_log_prices[
                    start_index
                ] + np.sum(permuted_changes[: i + 1])
    ...
    def generate_multi_market_permutation(
        self, price_data: List[pl.Series], start_index: Optional[int] = None
    ) -> List[pl.Series]:
        """
        ...
        for i, series in enumerate(price_data):
            if len(series) != n_bars:
                raise ValueError(
                    f"All price series must have same length. Series {i} has length {len(series)}, expected {n_bars}"
                )
    ...
    def generate_ohlc_permutation(
        self, ohlc_data: pl.DataFrame, start_index: Optional[int] = None
    ) -> pl.DataFrame:
        """
...
class PermutationTester:
    ...
    def run_insample_test(
        ...
        **strategy_kwargs,
        ...
        """
        logger.info(
            f"Starting in-sample permutation test with {self.config.n_permutations} permutations"
        )
        ...
        # Calculate p-value
        better_count = sum(
            1 for result in permutation_results if result >= real_performance
        )
        ...
        **strategy_kwargs,
        ...
        try:
            ...
            if hasattr(result, "backtest") and callable(result.backtest):
                ...
                raise ValueError(
                    f"Unsupported strategy function result type: {type(result)}"
                )
    ...
    def _run_parallel_permutations(
        ...
        **strategy_kwargs,
        ...
        n_workers = self.config.n_workers or min(
            mp.cpu_count(), self.config.n_permutations
        )
    ...
    def _run_sequential_walkforward_permutations(
        ...
        **strategy_kwargs,
        ...
        for i in range(self.config.n_permutations):
            if i % 50 == 0:
                logger.info(
                    f"Walk-forward permutation {i}/{self.config.n_permutations}"
                )
